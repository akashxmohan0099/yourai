    ...
    <div className="min-h-screen bg-[#f5f5f7]">
      <div className="space-y-6 max-w-7xl mx-auto px-4 py-6">
        <div className="flex items-center gap-4">
          <Link href="/clients" className="p-2.5 hover:bg-[#d2d2d7] bg-white border border-[#d2d2d7] rounded-xl transition-colors">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl font-semibold text-[#1d1d1f]">{client.name || 'Anonymous'}</h1>
                <CallClientButton
                  clientId={client.id}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            {/* Contact info */}
            <div className="bg-white rounded-2xl border border-[#d2d2d7] shadow-sm p-6 space-y-4">
              <h2 className="text-sm font-semibold text-[#86868b] uppercase tracking-wide">Contact</h2>
              {client.phone && (
                <div className="flex items-center gap-2.5 text-sm text-[#424245]">
                  <Phone className="w-4 h-4 text-[#86868b]" /> {client.phone}
                </div>
              )}
              {client.email && (
                <div className="flex items-center gap-2.5 text-sm text-[#424245]">
                  <Mail className="w-4 h-4 text-[#86868b]" /> {client.email}
                </div>
              )}
              {!client.phone && !client.email && (
                <p className="text-sm text-[#86868b]">No contact info</p>
              )}

              <h3 className="text-sm font-semibold text-[#86868b] uppercase tracking-wide pt-2">Channels</h3>
              {(client.channel_identities || []).map((ci: any, i: number) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  <span className="capitalize text-[#424245]">{ci.channel.replace('_', ' ')}</span>
                  <span className="text-xs text-[#86868b] font-mono">{ci.identifier}</span>
                </div>
              ))}
            </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-[#d2d2d7] shadow-sm p-6">
            <h2 className="text-sm font-semibold text-[#86868b] uppercase tracking-wide mb-4">
              ...
              <div className="divide-y divide-[#f5f5f7]">
                {conversations.map((conv: any) => (
                  <Link
                    ...
                    className="flex items-center gap-3 py-3.5 hover:bg-[#f5f5f7] -mx-3 px-3 rounded-xl transition-colors"
                  >
                    <div className="p-2 bg-[#f5f5f7] rounded-lg">
                      <MessageSquare className="w-4 h-4 text-[#86868b]" />
                    </div>
                    <div className="flex-1">
                      <span className="text-sm text-[#424245] capitalize font-medium">
                    ...
                    <span className="text-xs text-[#86868b]">{formatDateTime(conv.started_at)}</span>
                  </Link>
                ))}
              </div>
            )}
